...
class QuestDataset(torch.utils.data.Dataset):
    def __init__(self, df,  host_encoder=None, category_encoder=None, max_len=512, model_type = 'bert-base-cased', content="Question",augment = True,labeled=True, train_mode=True, extra_token=False):
        ...
        if ((self.model_type == 'bert-base-uncased') or (self.model_type == 'bert-large-uncased')):
            ...
            self.tokenizer = RobertaTokenizer.from_pretrained(self.model_type, additional_special_tokens = ["[UNK]","[SEP]","[PAD]","[CLS]","[MASK]"])
    ...
    def get_token_ids(self, row, index):
        ...
        if ('robert' in self.model_type):
            ...
            if self.content == "Question_Answer":
                if self.extra_token:
                    tokens = t_tokens + ["<sep>"] + q_tokens + ["<sep>"] + a_tokens + ["<sep>"] + ["<cls>"]+ ["<cls>"]+ ["<cls>"]
                else:
                    tokens = t_tokens + ["<sep>"] + q_tokens + ["<sep>"] + a_tokens + ["<sep>"] + ["<cls>"]
            elif self.content == "Question" or self.content == "Answer":
                if self.extra_token:
                    tokens = t_tokens + ["<sep>"] + c_tokens + ["<sep>"] + ["<cls>"]+ ["<cls>"]+ ["<cls>"]
                else:
                    tokens = t_tokens + ["<sep>"] + c_tokens + ["<sep>"] + ["<cls>"]
            else:
                raise NotImplementedError
