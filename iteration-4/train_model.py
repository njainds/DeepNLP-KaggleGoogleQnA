...
def training(content, n_splits, fold, model_type, model_name, hidden_layers,
             ...
             checkpoint_folder, load_pretrain, seed, loss, extra_token, augment, early_stopping, train_data_path, val_data_path, max_len, num_workers, host_encoder, category_encoder,len_trainloader):
    ...
    if model_type == "bert":
        if extra_token:
            model = QuestNet(model_type = model_name, n_classes=num_class, n_category_classes = num_category_class, n_host_classes = num_host_class, hidden_layers = hidden_layers, extra_token = True)
        else:
            model = QuestNet(model_type=model_name,  n_classes=num_class,n_category_classes=num_category_class, n_host_classes=num_host_class, hidden_layers=hidden_layers, extra_token=False)
    ...
    if optimizer_name == 'Adam':
        ...
        num_optimization_steps = num_epoch * len_trainloader//accumlation_steps
    ...
    if lr_scheduler_name =='CosineAnealing':
        num_train_optimization_steps = num_epoch * len_trainloader//accumlation_steps
    ...
    for epoch in range(1, num_epoch+1):
        ...
        for name,val in state_dict.items():
            state_dict[name].data.copy_((val.data + epoch*state_dict_last_epoch[name].data)/(epoch+1))
...
if __name__ == "__main__":
    ...
    len_trainloader = int(train_df.shape[0]/args.batch_size)
    training(args.content, args.n_splits, args.fold, args.model_type, args.model_name, args.hidden_layers, args.optimizer, args.lr_scheduler, args.lr, args.warmup_proportion, args.batch_size, args.valid_batch_size, args.num_epoch, args.start_epoch, args.accumulation_steps, args.checkpoint_folder, args.load_pretrain, args.seed, args.loss, args.extra_token, args.augment, args.early_stopping, train_data_path, val_data_path, args.max_len, args.num_workers,host_encoder,category_encoder, len_trainloader)
    gc.collect()
