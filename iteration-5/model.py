...
class QuestNet(nn.Module):
    ...
    def forward(self, ids, seg_ids):
        ...
        if 'roberta' in self.model_type:
            ...
            cls_outputs = torch.stack([dropout(hidden_states[i][:,0,:]) for i in self.hidden_layers], dim=2)
            cls_output = cls_outputs.sum(-1)/len(self.hidden_layers)
            #multisample dropout (wut): https://arxiv.org/abs/1905.09788
            #self.layer_weights are learnable weight torch.nn.Parameters(shape)
            #cls_output = (torch.softmax(self.layer_weights, dim=0) * cls_outputs).sum(-1)
            logits = torch.mean(torch.stack([self.fc(self.high_dropout(cls_output)) for _ in range(5)], dim=0 ), dim=0)
