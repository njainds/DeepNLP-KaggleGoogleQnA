...
parser.add_argument("--path_to_ckpt_config", type = str, default = str(os.getcwd()) + '/input/data/', required=False, help="specify folder path of train.csv")
...
if __name__ == "__main__":
    ...
    log.write("is_distributed is %s" %str(is_distributed))
    ...
    log.write("device: {} n_gpu: {}, distributed training: {}".format(device, n_gpu, bool(is_distributed)))
    accumlation_steps = 2
    #args.batch_size = int(args.batch_size / accumlation_steps)
    #batch size is actually the miniacth size esample: total bacth size is 128 with 2 gpus each on 4 nodes. so batch size is 128/(2*4)=16
    ...
    optimizer_grouped_parameters.append({'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)],'lr': args.lr, 'weight_decay': 0.01})
    optimizer_grouped_parameters.append({'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)],'lr': args.lr, 'weight_decay': 0.0})
    ...
    if args.lr_scheduler_name == "WarmupLinearSchedule":
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer,num_warmup_steps=int(warmup_proportions * t_total),num_training_steps=t_total)
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for tr_batch_i, (inputs,targets) in enumerate(train_loader):
            ...
            targets_mlm, targets_sop, targets_extra = targets_mlm.to(device), targets_sop.to(device), targets_extra.to(device)
            model.train()
            logits = model(input_ids, token_type_ids, attention_mask)
            logits_mlm, logits_sop, logits_extra = logits
            logits_mlm, logits_sop, logits_extra = logits_mlm.to(device), logits_sop.to(device), logits_extra.to(device)
            ...
            if (tr_batch_i + 1) % log_step == 0:
                ...
                log.write("\t Batch # %d \t perc : %f \t  combined_loss: %f \t mlm_loss: %f \t sop_loss: %f \t mlm_perplexity: %f \t sop_accuracy: %f \t elapsed time: %d\n" %((tr_batch_i + 1), ((tr_batch_i + 1) / len(train_loader)), avg_loss, avg_mlm_loss, avg_sop_loss,avg_mlm_perplexity, avg_sop_acc, elapsed_time))
        ...
        ckpt_filename = "ckpt_model_epoch_" + str(epoch) + "_rank_" + str(int(os.environ['RANK'])) + "_" + "_seed_" +  str(args.seed) + ".pth"
