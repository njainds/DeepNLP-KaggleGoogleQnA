...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for tr_batch_i, (inputs, targets) in enumerate(train_loader):
            ...
            mlm_perplexity = MLMPerplexity(ignore_index=-1)(
                (logits_mlm.cpu().detach(), logits_sop.cpu().detach(), logits_extra.cpu().detach()),
                (targets_mlm.cpu().detach(), targets_sop.cpu().detach(), targets_extra.cpu().detach()))
            sop_acc = sop_accuracy((logits_mlm.cpu().detach(), logits_sop.cpu().detach(), logits_extra.cpu().detach()),
                                   (targets_mlm.cpu().detach(), targets_sop.cpu().detach(),
                                    targets_extra.cpu().detach()))
            ...
            if (tr_batch_i + 1) % log_step == 0:
                ...
                log.write(
                    ...
                    avg_mlm_perplexity, avg_sop_acc, elapsed_time))
            ...
            if (tr_batch_i + 1) % eval_step == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch_i, (inputs, targets) in enumerate(val_loader):
                        ...
                        logits_mlm, logits_sop, logits_extra = logits
                        logits_mlm, logits_sop, logits_extra = logits_mlm.cuda(), logits_sop.cuda(), logits_extra.cuda()
                        loss = criterion((logits_mlm, logits_sop, logits_extra),
                                         (targets_mlm, targets_sop, targets_extra))
