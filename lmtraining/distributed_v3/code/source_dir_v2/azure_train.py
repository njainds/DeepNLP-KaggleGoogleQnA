            ...
            if (tr_batch_i + 1) % eval_step == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch_i, (inputs, targets) in enumerate(val_loader):
                        ...
                        # calculating metrics for logging
                        avg_mlm_loss += mlm_loss.item()/len(v_sampler)
                        avg_sop_loss += sop_loss.item()/len(v_sampler)
                        avg_mlm_perplexity += mlm_perplexity/len(v_sampler)
                        avg_loss += loss.item()/len(v_sampler)
                        avg_sop_acc += sop_acc/len(v_sampler)
