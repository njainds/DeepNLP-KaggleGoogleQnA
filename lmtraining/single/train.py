...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for tr_batch_i, (inputs,targets) in enumerate(train_loader):
            ...
            if tr_batch_i == 0:
                s_logits_mlm,s_logits_sop,s_logits_extra = logits[0].cpu().detach(),logits[1].cpu().detach(),logits[2].cpu().detach()
                s_target_mlm, s_target_sop, s_target_extra = targets[0].cpu().detach(), targets[1].cpu().detach(), targets[2].cpu().detach()
                avg_loss = loss.item()

            else:
                s_logits_mlm = torch.cat((s_logits_mlm, logits[0].cpu().detach()),dim=0)
                s_logits_sop = torch.cat((s_logits_sop, logits[1].cpu().detach()),dim=0)
                s_logits_extra = torch.cat((s_logits_extra, logits[2].cpu().detach()),dim=0)
                s_target_mlm = torch.cat((s_target_mlm, targets[0].cpu().detach()),dim=0)
                s_target_sop = torch.cat((s_target_sop, targets[1].cpu().detach()),dim=0)
                s_target_extra = torch.cat((s_target_extra, targets[2].cpu().detach()),dim=0)
                avg_loss = (loss.item() + avg_loss*(tr_batch_i))/(tr_batch_i+1)

            s_logits = (s_logits_mlm,s_logits_sop,s_logits_extra)
            s_targets = (s_target_mlm,s_target_sop,s_target_extra)
            mlm_loss = MLMloss(ignore_index=-1)(s_logits[0],s_targets[0])
            sop_loss = SOPloss()(s_logits[1],s_targets[1])
            mlm_perplexity = MLMPerplexity(ignore_index=-1)(s_logits,s_targets)
            sop_acc = sop_accuracy(s_logits,s_targets)
            ...
            if (tr_batch_i + 1) % log_step == 0:
                ...
                log.write("\t Batch # %d \t perc : %f \t  combined_loss: %f \t mlm_loss: %f \t sop_loss: %f \t mlm_perplexity: %f \t sop_accuracy: %f \t elapsed time: %d\n" %
                          ((tr_batch_i + 1), ((tr_batch_i + 1) / len(train_loader)), combined_loss,mlm_loss,sop_loss,mlm_perplexity,sop_acc,elapsed_time))
            ...
            if (tr_batch_i + 1) % eval_step == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch_i, (inputs, targets) in enumerate(val_loader):
                        ...
                        # calculating metrics for logging
                        if val_batch_i == 0:
                            v_logits_mlm, v_logits_sop, v_logits_extra = logits[0].cpu().detach(), logits[1].cpu().detach(), logits[2].cpu().detach()
                            v_target_mlm, v_target_sop, v_target_extra = targets[0].cpu().detach(), targets[1].cpu().detach(), targets[2].cpu().detach()
                            avg_loss = loss.item()

                        else:
                            v_logits_mlm = torch.cat((v_logits_mlm, logits[0].cpu().detach()), dim=0)
                            v_logits_sop = torch.cat((v_logits_sop, logits[1].cpu().detach()), dim=0)
                            v_logits_extra = torch.cat((v_logits_extra, logits[2].cpu().detach()), dim=0)
                            v_target_mlm = torch.cat((v_target_mlm, targets[0].cpu().detach()), dim=0)
                            v_target_sop = torch.cat((v_target_sop, targets[1].cpu().detach()), dim=0)
                            v_target_extra = torch.cat((v_target_extra, targets[2].cpu().detach()), dim=0)
                            avg_loss = (loss.item() + avg_loss * (val_batch_i)) / (val_batch_i + 1)

                        v_logits = (v_logits_mlm, v_logits_sop, v_logits_extra)
                        v_targets = (v_target_mlm, v_target_sop, v_target_extra)
                        mlm_loss = MLMloss(ignore_index=-1)(v_logits[0], v_targets[0])
                        sop_loss = SOPloss()(v_logits[1], v_targets[1])
                        mlm_perplexity = MLMPerplexity(ignore_index=-1)(v_logits, v_targets)
                        sop_acc = sop_accuracy(v_logits, v_targets)
                        combined_loss = avg_loss
                        ...
                        if (val_batch_i + 1) % len(val_loader) == 0:
                            ...
                            log.write("\t Batch # %d \t perc : %f \t  combined_loss: %f \t mlm_loss: %f \t sop_loss: %f \t mlm_perplexity: %f \t sop_accuracy: %f \t elapsed time: %d\n" %
                                ((val_batch_i + 1), ((val_batch_i + 1) / len(val_loader)), combined_loss, mlm_loss,sop_loss, mlm_perplexity, sop_acc, elapsed_time))
